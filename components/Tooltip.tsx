...
const Tooltip: React.FC<TooltipProps> = ({ node, position, onDrillDown, onMouseEnter, onMouseLeave }) => {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-50/50 p-3 rounded border border-slate-100">
        <div className="flex justify-between items-end mb-2">
            <div>
                <p className="text-xs text-slate-500 uppercase font-bold">实时库存监控 (14天趋势)</p>
                <div className="flex items-baseline gap-1 mt-1">
                    <span className={`text-2xl font-bold ${node.status === 'critical' ? 'text-red-600' : 'text-slate-700'}`}>
                        {node.inventoryLevel?.toLocaleString()}
                    </span>
                    <span className="text-sm text-slate-400">Ah</span>
                </div>
            </div>
            <div className="text-right">
                <p className="text-xs text-slate-400">库容占用</p>
        ...
        {node.status === 'critical' && (
            <div 
                ...
                className="mt-2 flex items-start gap-1.5 text-xs text-red-600 bg-red-50 hover:bg-red-100 p-2 rounded cursor-pointer transition-colors border border-transparent hover:border-red-200 group"
            >
                <AlertCircle size={14} className="mt-0.5 flex-shrink-0" />
                <div className="flex-1">
                    <span>库存触及红线，点击查看溯源分析。</span>
                </div>
                <ZoomIn size={14} className="mt-0.5 opacity-0 group-hover:opacity-100 text-red-400" />
            </div>
        )}
      </div>
      ...
      <div>
        <p className="text-xs text-slate-400 uppercase font-bold mb-2 flex items-center gap-1">
            <Factory size={12} /> 产线运行状态 (OEE & 良率)
        </p>
        <div className="grid gap-2">
            {node.productionLines?.map(line => (
                <div key={line.id} className="flex items-center justify-between bg-white border border-slate-200 p-2.5 rounded shadow-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <div className="text-sm font-bold text-slate-700">{line.name}</div>
                    ...
                    <div className="flex gap-4 text-right">
                         <div>
                            ...
                            <div className="text-sm font-mono font-medium text-blue-600">{line.efficiency.toFixed(1)}%</div>
                         </div>
                         <div>
                            <div className="text-[10px] text-slate-400">良率</div>
                            <div className="text-sm font-mono font-medium text-emerald-600">{line.yieldRate.toFixed(1)}%</div>
                         </div>
                    </div>
                </div>
            ))}
  ...
  const renderCustomerContent = () => (
    <div className="space-y-4">
      ...
      <div className="bg-slate-50/50 p-3 rounded border border-slate-100">
         <div className="flex justify-between items-center mb-2">
             <p className="text-xs text-slate-500 uppercase font-bold flex items-center gap-1">
                 <TrendingUp size={12} /> 需求预测 vs 实际交付
             </p>
      ...
      <div>
         <p className="text-xs text-slate-400 uppercase font-bold mb-2 flex items-center gap-1">
            <Package size={12} /> 重点在产订单跟踪
        </p>
        <div className="bg-white border border-slate-200 rounded overflow-hidden">
            <table className="w-full text-left">
                <thead className="bg-slate-50 text-[10px] text-slate-500 font-medium">
                    <tr>
                        <th className="px-3 py-2">订单号/产品</th>
                        <th className="px-3 py-2 text-center">排产进度</th>
                        <th className="px-3 py-2 text-right">交付日</th>
                    </tr>
                </thead>
                    ...
                    {node.activeOrders?.slice(0, 3).map(order => (
                        ...
                        >
                            <td className="px-3 py-2">
                                ...
                                <div className="text-[10px] text-slate-400 scale-95 origin-left">{order.product}</div>
                            </td>
                            <td className="px-3 py-2 align-middle">
                                <div className="w-full bg-slate-100 rounded-full h-2 mb-0.5">
                                    <div 
                                        className={`h-2 rounded-full ${order.status === 'delayed' ? 'bg-red-400' : 'bg-emerald-400'}`} 
                                        style={{width: `${order.progress}%`}}
                                    ></div>
                                </div>
                                <div className="text-[10px] text-center text-slate-500">{order.progress}%</div>
                            </td>
                            <td className="px-3 py-2 text-right whitespace-nowrap">
                                <div className={order.status === 'delayed' ? 'text-red-600 font-bold' : 'text-slate-600'}>
                                    {order.dueDate.slice(5)}
                                </div>
                                {order.status === 'delayed' && <div className="text-[10px] text-red-500 flex items-center justify-end gap-1">已延期 <ZoomIn size={10}/></div>}
                            </td>
                        </tr>
                    ))}
                </tbody>
      ...
      {/* 3. Supplying Bases */}
      <div>
         <p className="text-xs text-slate-400 uppercase font-bold mb-1.5 flex items-center gap-1">
             <Truck size={12} /> 供应协同基地
         </p>
         <div className="flex flex-wrap gap-1.5">
             {node.supplyingBases?.slice(0, 4).map(base => (
                 <span key={base} className="px-2 py-1 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded text-[10px] font-medium">
                     {base}
                 </span>
             ))}
             {(node.supplyingBases?.length || 0) > 4 && <span className="text-xs text-slate-400 self-center">...</span>}
         </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h3 className={`text-base font-bold ${node.status === 'critical' ? 'text-red-800' : 'text-slate-800'}`}>{node.name}</h3>
          <p className="text-xs text-slate-500 font-medium tracking-wide uppercase flex items-center gap-1 mt-0.5">
              {node.type === NodeType.BASE ? <Factory size={12}/> : node.type === NodeType.CUSTOMER ? <Activity size={12}/> : <Truck size={12}/>}
        ...
        {node.activeAlerts && node.activeAlerts > 0 ? (
           ...
           >
                <span className="flex items-center gap-1 text-xs font-bold text-white bg-red-500 px-2.5 py-0.5 rounded-full shadow-sm group-hover:bg-red-600 transition-colors">
                    <AlertTriangle size={12} /> {node.activeAlerts}项风险
                </span>
                <span className="text-[10px] text-red-600 mt-1 font-medium flex items-center gap-1">
                    点击查看分析 <ZoomIn size={10}/>
                </span>
           </div>
        ) : (
         ...
         node.type === NodeType.CUSTOMER ? renderCustomerContent() : (
            ...
            <div className="space-y-4">
                <div className="bg-slate-50 p-3 rounded border border-slate-100 grid grid-cols-2 gap-3">
                    <div>
                        <div className="text-xs text-slate-400 uppercase">供应物料库存</div>
                        <div className="text-xl font-bold text-slate-700">{node.inventoryLevel?.toLocaleString()} T</div>
                    </div>
                     <div>
                        <div className="text-xs text-slate-400 uppercase">交付准时率</div>
                        <div className="text-xl font-bold text-emerald-600">{node.deliveryAccuracy?.toFixed(1)}%</div>
                    </div>
                </div>
