  ...
  return (
    <div className="flex flex-col h-full bg-white w-full">
       <div className="p-4 border-b border-slate-200 flex items-center gap-2 bg-gradient-to-r from-slate-50 to-white sticky top-0 z-10">
        ...
        <div>
          <h2 className="font-bold text-slate-800 text-base">智能决策助手</h2>
          <p className="text-xs text-slate-400">Interactive Simulation Agent</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/30">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'model' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'}`}>
              {msg.role === 'model' ? <Bot size={20} /> : <User size={20} />}
            </div>
            <div className={`max-w-[85%] rounded-2xl px-5 py-3 text-base shadow-sm ${
              msg.role === 'model' 
      ...
      <div className="p-4 border-t border-slate-200 bg-white sticky bottom-0 z-10">
        <div className="relative">
          <input
            type="text"
            className="w-full bg-slate-50 border border-slate-200 text-slate-800 text-base rounded-lg pl-4 pr-12 py-3.5 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
            placeholder="输入指令..."
            value={input}
