...
const DEFAULT_LLM_CONFIG: LLMConfig = {
    provider: 'kimi',
    apiKey: 'sk-cB9rX5ktOfGnUYXI8oI5qTCsFdknxamdmbc6sf9HPin2Z7v9',
    baseUrl: 'https://api.moonshot.cn/v1',
    modelName: 'kimi-k2.5' // As requested
};
...
function App() {
  ...
  const callAI = async (prompt: string, tools?: any[]) => {
      ...
      if (llmConfig.provider === 'gemini') {
          // Fallback to process.env if user hasn't set one in UI but it's available globally
          const apiKey = llmConfig.apiKey || process.env.API_KEY;
          
          if (!apiKey) {
              return { text: "⚠️ 请先在设置页面配置 Gemini API Key (或检查环境变量)。", functionCalls: undefined };
          }

          try {
              const ai = new GoogleGenAI({ apiKey: apiKey });
              const modelId = llmConfig.modelName || 'gemini-2.5-flash';
